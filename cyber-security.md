## Networking
- Networking Concepts Beej
- From Zero To HTTP - https://www.youtube.com/watch?v=9uebakqWlB0
- OSI and TCP/IP Models Best Explained - https://www.youtube.com/watch?v=3b_TAYtzuho
- https://study-ccna.com/

## Linux / Sysadmin
- TCM Security
- Tmux in 100 seconds - https://www.youtube.com/watch?v=vtB1J_zCv8I
- Complete Bash Scripting Course - https://www.youtube.com/watch?v=Sx9zG7wa4FA
- Effective Shell - https://effective-shell.com/introduction
- https://biriukov.dev/
- https://vim-adventures.com/
- https://linuxupskillchallenge.com/
- https://linuxjourney.com/
- https://sadservers.com/
- https://www.docker.com/101-tutorial/

## Challenges
- OverTheWire
- TryHackMe
- HackTheBox
- Crypto and Forensics Practice

## Red Teaming
Predominantly composed of two things: alternative analysis and adversary simulation. Red teams do not attempt to find “all the vulnerabilities'' and do not usually try to have a wide breadth of coverage. Instead, red teams seek to simulate an adversary with a particular objective, predominantly to act as a “sparring partner” for blue teams. Keep in mind, red teams are the only adversary that will debrief with the blue team so that the blue team can figure out what they missed or could have done differently.
- Carlos Polop - AWS Red Team Expert
- Threat Adversary Emulation - https://www.youtube.com/watch?v=783BgVwzr4Y
- Akamai - https://www.linode.com/docs/guides/linux-red-team-exploitation-techniques/
- CI/CD CTF - https://github.com/cider-security-research/cicd-goat
- Windows Access Tokens - https://x.com/aceresponder/status/1808287286922301840?s=46&t=wDGoNTlzP8Yj3u5OvZvcNQ
- https://usacac.army.mil/sites/default/files/documents/ufmcs/The_Red_Team_Handbook.pdf
- https://www.slideshare.net/TobyKohlenberg/red-teaming-probably-isnt-for-you-81283357
- https://www.youtube.com/watch?v=mj2iSdBw4-0
- https://systemoverlord.com/2017/09/18/getting-started-in-offensive-security.html#understanding-vulnerabilities


## Threat Intel
- https://cloud.google.com/learn/security/mandiant-academy-courses/threat-hunting
- https://www.reddit.com/r/cybersecurity/comments/1bdvdm2/threat_intel_threat_research_cybersecurity/
- https://cloud.google.com/blog/topics/threat-intelligence/cti-analyst-core-competencies-framework/
- https://cloud.google.com/blog/topics/threat-intelligence/unc4393-goes-gently-into-silentnight
- https://cloud.google.com/blog/topics/threat-intelligence/lummac2-obfuscation-through-indirect-control-flow

## Pentesting / Cloud / Blue Team / IT / OSINT
- Loi Liangyang
- TraceLabs
- Shodan - search engine 
- Flaws Cloud - Challenges
- Pentest Ground
- Pentest Tools
- Entire Cyber Security Degree in 15 mins - https://www.youtube.com/watch?v=AhMSK5GwckU
- Symone B - https://www.youtube.com/watch?v=9JKiITtE01s&t=21s
- TechTual Chatter - https://www.youtube.com/watch?v=sdQDTh82cJU
- https://github.com/NextSecurity/Awesome-Cloud-Security
- https://pentesterlab.com/
- https://cyberdefenders.org/
- https://www.youtube.com/@MadeByGPS/videos
- https://cybersecurity.att.com/blogs/security-essentials/theres-no-such-thing-as-an-entry-level-job-in-cybersecurity
- https://github.com/sherlock-project/sherlock
- https://www.crowdstrike.com/cybersecurity-101/threat-intelligence/
- https://www.reddit.com/r/AskNetsec/comments/u8aetw/advice_on_getting_more_interviews_for_threat/


## DFIR
SOAR EDR Project - https://www.youtube.com/watch?v=OirFGI-34Ko
https://github.com/Azr43lKn1ght/DFIR-LABS
https://start.me/p/q6mw4Q/forensics

## Certifications
- Blurb from Bobby Cooke @ IBM Red Team: From my XP, for Red Team engagements you are going to be going up against clients that are mature enough to do Red Team engagements. This means that they will probably have Active Directory, and you will be going up against millions of dollars of security systems and a CSOC/blue-team. IMO you're going to have to code your own tools or heavily modify existing. So pretty much we need the operator - skills from CRTO/CRTP tool dev skills from OSEP/Sektor7, and maldev skills from Sektor7.
    1. OSCP (for netpen basics)
    2. CRTO (Learn Red Teaming zero>Hero) - https://training.zeropointsecurity.co.uk/courses/red-team-ops
    3. CRTP (Red Teaming LOL style & PowerShell)
    4. OSEP (Red Team tooldev & nailing down C#)
    5. @SEKTOR7net (Red Team C tooldev & malware Dev)
- Blue Team - CyberDefender
- GIAC - Exploit Dev
- GIAC - GREM
- SANS - FOR508 / FOR610
- CRTO - ZeroPoint
- SANS - Cyber Threat Intel, Threat Hunting, Incidence Response, Digital Forensics, Red Team Ops Adversary Emulation, Enterprise Pen Testing
- Blurb: Why I like OSCP as a baseline certificate for roles in my team: to have it, you need a variety of skills that are useful in my line of work. You need:
    - a bit of networking
    - a bit of binary analysis
    - a bit of intrusion methodology a bit of intrusion techniques
    - a bit of information gathering
    - a bit of exploitation, a bit of OS, a bit of tenacity, a bit of scripting

